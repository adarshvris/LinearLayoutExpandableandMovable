...
public class TrayAnimation {
    ...
    private TrayAnimation(AnimationComposer animationComposer) {
    ...
    public static final class AnimationComposer {
        ...
        private float pivotX = TrayAnimation.CENTER_PIVOT, pivotY = TrayAnimation.CENTER_PIVOT;
        private Interpolator interpolator;
        private View target;
        ...
        public YoYoString playOn(View target) {
            this.target = target;
            return new YoYoString(new TrayAnimation(this).play(), this.target);
        }
    ...
    private BaseViewAnimator play() {
        ...
        if (pivotX == TrayAnimation.CENTER_PIVOT) {
            ViewCompat.setPivotX(target, target.getMeasuredWidth() / 2.0f);
        } else {
            target.setPivotX(pivotX);
        }
        if (pivotY == TrayAnimation.CENTER_PIVOT) {
